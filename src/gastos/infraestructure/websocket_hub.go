...
// WebSocketMessage estructura para mensajes en WebSocket
type WebSocketMessage struct {
	Type    string      `json:"type"` // "create" | "update" | "delete" | "subscribe"
...
func (h *Hub) run() {
	for {
		...
		case client := <-h.register:
			...
			h.mu.Unlock()
			log.Printf("Cliente registrado. Total de clientes: %d", len(h.clients))
		...
		case client := <-h.unregister:
			...
			h.mu.Unlock()
			log.Printf("Cliente desregistrado. Total de clientes: %d", len(h.clients))
...
// BroadcastEvent envía un evento a todos los clientes suscritos
func (h *Hub) BroadcastEvent(msgType string, grupoID int32, data interface{}) {
